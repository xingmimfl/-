...
- fit_generator
- train_on_batch
如果想要自己写dataset和dataloader, 那么需要使用后面两种方法
...
landmarks_model.compile(loss=mse_loss, optimizer=optimizer) #----这个地方,loss=mse_loss似乎可以不用写，我记得没写的时候运行结果也是正确的
...
for  i in range(MAX_EPOCH):
    ...
    for i_batch, sample_batched in enumerate(train_loader):
        with tf.GradientTape() as tape:
            ...
            ...
            loss =mse_loss(input, label) #---这个地方使用自定义的loss函数
            ...
            ...
```
...
[ref1](https://towardsdatascience.com/learning-rate-schedule-in-practice-an-example-with-keras-and-tensorflow-2-0-2f48b2888a0c#:~:text=The%20constant%20learning%20rate%20is%20the%20default%20schedule,SGD%20optimizer%20and%20pass%20the%20argument%20learning_rate%3D0.01%20), [ref2](https://blog.csdn.net/qq_36556893/article/details/103645204)，[ref3](https://devdocs.io/tensorflow~2.3/keras/callbacks/learningratescheduler)
...
class AvgPool(keras.layers.Layer):
    ...
    def get_config(self):
        ...
        return config
```
这里我们首先要继承keras.layers.Layer这个方法。这里有一个函数叫做get_config(), 如果没有这个函数的话,那么保存模型参数h5之后再读取这个h5的时候就会报一些错误
...
```
if instance(layer, nn.BatchNorm2d):
    layer.eval()
```
